...
const ThemeContext = createContext<SimpleThemeContextType | undefined>(
  undefined,
);
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  const [preferences, setPreferences] = useState<SimpleUserPreferences | null>(
    null,
  );
  ...
  const getEffectiveTheme = useCallback(
    (mode: SimpleThemeMode): "light" | "dark" => {
      if (mode === "system") {
  ...
  const loadPreferences = useCallback(async () => {
    ...
    setLoading(true);
    try {
      let prefs: SimpleUserPreferences | null = userRef.current
        ? await ThemeService.getUserPreferences()
        : null;
      if (!prefs) {
